...
class BarChartDataPlotter(DataPlotter):
    def plot_batch(self, train_set_inputs, train_set_output, synthetic_output, epoch, output_dir):
        ...
        for idx, sample in enumerate(synthetic_output):
            ...
            for ti in range(train_set_inputs.shape[1]):
                ...
                plt.bar(x_input, train_input, label='Input_{}'.format(ti), color='indigo')
            ...
            plt.bar(x_output - width/2, sample, width=width, label='G_output', color='dodgerblue')
            plt.bar(x_output + width/2, train_set_sample, width=width, label='Validation_Output', color='mediumvioletred')
