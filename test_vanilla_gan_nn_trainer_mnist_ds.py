...
class MnistDataSet(Dataset):
    def __init__(self):
        transform = transforms.Compose([transforms.ToTensor(), transforms.Normalize(mean=(0.5), std=(0.5)), transforms.Resize((img_height_width, img_height_width))])
    ...
    def __getitem__(self, index):
        ...
        z = torch.randn(z_size).numpy()
        r = numpy.concatenate(([input], z))
        #z[0] = float(input)
        return r, output.numpy()#0.5*(output.numpy() + 1)
...
trainer = VanillaGanNnTrainer(generator_neural_network=netG, discriminator_neural_network=netD, verbose=True, max_epoch_count=args["num_epochs"],batch_size=args["batch_size"], output_dir=OUTPUT_PATH, logger=LOGGER)
